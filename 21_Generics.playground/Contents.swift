...
func swapTwoInts(inout a: Int, inout b: Int) -> Void {
    let tempA = a
    a = b
    b = tempA
}

//这个函数使用写入读出（in-out）参数来交换a和b的值.
...
swapTowValues(&someInt, b: &anotherInt)
...
let doubleIndex = findIndex([3.14,0.1,0.44], valueToFind: 9.3)
...
protocol Container {
    typealias ItemType
    mutating func append(itme: ItemType)
    var count: Int {get}
...
/*
 ...
 必须可以通过append方法添加一个新item到容器里；
 必须可以通过使用count属性获取容器里items的数量，并返回一个Int值；
 必须可以通过容器的Int索引值下标可以检索到每一个item。
 */
...
var astackOfStrings = NewStack<String>()
astackOfStrings.push("uno")
astackOfStrings.push("dos")
stackOfStrings.push("tres")

var arrayOfStrings = ["uno","dos","tres"]

if allItemsMatch(astackOfStrings, anotherContainer: arrayOfStrings) {
